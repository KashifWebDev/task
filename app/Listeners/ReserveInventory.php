...
class ReserveInventory implements ShouldQueue
{
    ...
    public function handle(OrderConfirmed $event): void
    {
        ...
        try {
            DB::transaction(function () use ($orderId, &$attempt) {
                ...
                if ($order->hasStepSucceeded('inventory')) {
                    Log::info("Inventory already reserved for order {$order->order_number}");
                    return;
                }

                $attempt = $order->getLatestAttemptForStep('inventory') + 1;

                if (rand(1, 100) <= 10) {
                    throw new \RuntimeException('Inventory reservation failed: Insufficient stock');
                }
                ...
                Log::info("Inventory reserved for order {$order->order_number} (attempt {$attempt})");

                $this->checkOrderCompletion($order);
            });
        } catch (\Throwable $e) {
            $this->recordFailure($orderId, $attempt, 'inventory', $e);
            throw $e;
        }
    }
    ...
    {
        $order = $event->order;
        Log::error("ReserveInventory job failed for order {$order->order_number}: {$exception->getMessage()}");
    }
    ...
    protected function recordFailure(int $orderId, ?int $attempt, string $step, \Throwable $exception): void
    {
        DB::transaction(function () use ($orderId, $attempt, $step, $exception) {
            ...
            Log::error("Inventory reservation failed for order {$order->order_number}: {$exception->getMessage()}");
        });
    }
}
